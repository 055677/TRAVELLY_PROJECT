...
const App = () => {
  ...
  const handleCostSubmit = async (values) => {
    // Ensure num_children is set to 0 for 'solo' and 'couple' vacation types
    if (vacationType === 'solo' || vacationType === 'couple') {
      values.num_children = 0;
    }
  
    // Convert values to integers
    values.num_days = parseInt(values.num_days, 10);
    values.num_children = parseInt(values.num_children, 10);
  ...
  // State to track the selected vacation type
  const [vacationType, setVacationType] = useState('');

  // Handle the change of the vacation type selection
  const handleVacationTypeChange = (value) => {
    setVacationType(value);
    message.info(
      value === 'solo'
        ? 'Solo trips are all about self-discovery! 🌟'
        : value === 'couple'
        ? 'A romantic getaway awaits! 💕'
  ...
  return (
    <Layout className="app-layout">
      <Header className="header">
         <div className="header-title">Travel Recommender 🌍✈️</div>
      ...
      <Content className="content">
        <Card className="about-card" title="Discover the World with Us 🌍" id="about">
          <p>
            Our mission is to inspire your next adventure! With personalized recommendations, cost estimations, and city insights, your dream trip is just a few clicks away.
          </p>
        </Card>
        ...
        <Card className="contact-card" title="Contact Us 📞" id="contact">
          ...
          <p>
            Reach out to us for travel tips, support, or just to share your favorite destinations!
          </p>
          <p>Email: wanderlust@travelrecommendations.com</p>
        ...
        <Card title="Get Recommendations" id="recommendations" className="card">
          ...
          {recommendation && (
            <p>
              <strong>Your Dream Destination:</strong> {recommendation}! Pack your bags! 🧳
            </p>
          )}
        </Card>
        ...
        <Card title="Calculate Trip Cost" className="card">
          <Form onFinish={handleCostSubmit}>
            ...
            >
              <Select className="select-field" onChange={handleVacationTypeChange} value={vacationType}>
                <Option value="solo">Solo</Option>
                <Option value="couple">Couple</Option>
                <Option value="family">Family</Option>
              </Select>
            </Form.Item>
            { vacationType === 'family' && (<><Form.Item
              name="num_children"
      ...
      <Footer className="footer">
        ...
        <br />
        Have Questions? Reach Out to Us! ✈️
      </Footer>
